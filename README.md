...
制作可供其他 Mac 用户安装的 `.pkg` 安装包：

```bash
./build.sh --pkg
```

产物为同目录的 `键盘热力图-安装包.pkg`，安装后 app 会放入「应用程序」文件夹。项目没有购买 Apple Developer Program 的 Developer ID 签名和公证，所以从 GitHub 下载的用户首次安装或打开时，macOS 可能提示“无法验证开发者”；可在「系统设置 › 隐私与安全性」中选择“仍要打开”。这是未付费分发 macOS app 的系统限制，普通 `.pkg` 无法绕过。
...
- 可在右侧「运行设置」或菜单栏菜单中开启「开机时自动启动」，也可在系统设置的「通用 › 登录项」中关闭。
